...
//        supportActionBar?.setDisplayHomeAsUpEnabled(true)
        ...
        calenderPrevDate.add(Calendar.MONTH, -1)

        currentPrevDate = formatter.format(calenderPrevDate.time)
        Log.i("CALL_LOG", "Prev One Mi=onth :" + currentPrevDate)
    ...
    inner class LoadCallLogTask : AsyncTask<Void, Void, Void>() {
        ...
        override fun onPostExecute(result: Void?) {
           ...
           txtCount.setText("" + callLogList?.size ?: "0")
            Log.i("CALL_LOG", Gson().toJson(callLogList)).toString()

          //  showAlert("Dtata:"+ Gson().toJson(callLogList)).toString()

          //  jsonAlert(""+ Gson().toJson(callLogList).toString() )
            ...
            Log.d("COUNT", "Total Count are " + callLogList?.size ?: "0")
